...
/**
 * Class for header of {@code .DAT} file from the radar, adapted from the Auto-pRES Manual page
 * 12
 */
public class Header {
  ...
  /**
   * Returns the ASCII representation of the header directly from the file. may not be necessary but
   * available for raw output.
   *
   ...
   * Ethernet port is detected. This can be useful if the laptop keeps entering sleep mode, in which
   * case the Ethernet port will be seen by the radar as inactive.
   *
   ...
   * attempted every Ndata bursts. So NData=1 will cause an uplink every burst. NData=24 will cause
   * a daily uplink if RepSecs is set to 3600 seconds. NData=0 will disable data uplinks.
   *
   * @return nData
   */
  public int getNData() {
   ...
   * <p>From manual page 11:
   *
   * <p>Defines which depth intervals are reported in the data uplinks. This consists of a series of
   * concatenated triplets of numbers on one line. Eg Triples=200,15,300,300,25,600,650,4,690. Each
   * triple is treated in a similar way to the Matlab xmin:dx:xmax syntax. So the first set of depth
   * intervals in the example would be 200 to 215 m, 215 to 230 m, 230 to 245 m etc up to 275 to 290
   * m. The maximum number of triples is 4, and the maximum number of depth intervals is 64. The
   * example would report on 28 intervals in total. See section on data uptells for further details,
   * and for the content of the data messages
   *
   * @return triples
   */
   ...
   *
   * <p>If IntervalMode = 0, ensure that the repetition interval is greater than the length of time
   * for the acquisition of an entire burst. If it is shorter, then the radar will continue with an
   * uninterrupted series of bursts, and it is likely that it will not be possible to switch off the
   * unit without losing the final open data file. To estimate the length of the burst, assume that
   * there will be at least 10 seconds of overheads associated with starting up the radar. So the
   * total time = NSubBursts * nAttenuators + 10 seconds. Setting IntervalMode to 1 means that a
   * RepSecs-long delay will be implemented between bursts. If the total time for a burst lasts
   * longer than the Watchdog timeout interval, which defaults to 3600 seconds, the system will
   * automatically reset itself. This would effectively limit the number of chirps per burst to
   * around 2180. Note that the Watchdog Timer timeout can be adjusted with the WATCHDOC_TASK_SECS
   * value, and that the Watchdog is disabled during Attended Mode operation.
   *
   * @return repSecs
   */
   ...
   *
   * <p>If set to zero, the Ethernet port is not checked on power-up. This defaults to one. Probably
   * best left that way.
   *
   ...
   * them. Each time unattended operation is initiated a new subdirectory is created on the card to
   * hold the files associated with the sequence of bursts
   *
   ...
   * chirps in a sub-burst (assuming not in MIMO mode – see below). If averaging is enabled (Average
   * ≠ 0), all chirps in a burst are averaged together. Averaging should not be used with when
   * multiple attenuator settings are used
   *
   * @return nAttenuators
   */
   ...
   * activities will take place at the end of the first burst after midnight. Iridium communications
   * will be attempted (see below), and the SD cards will be checked and any detected errors will be
   * repaired.
   *
   * @return housekeeping
   */
   ...
   * initiated, the GPS module is activated. GPSon then indicates the time in seconds allowed for a
   * fix to be obtained. If successful, the GPS location, GPS time and the radar’s system time at
   * the time of the fix are stored in the data header for the burst on the SD card. Should be set
   * to zero if no GPS antenna is connected, or if there is some other reason to think a fix will
   * not be obtained. The module remembers the satellite constellation between fixes: after a fix
   * has been obtained, the module usually requires only a few seconds to reacquire the satellites.
   * So even if a long period is allowed by GPSon (eg 30 seconds), it would be expected to take only
   * a few seconds before each burst to get a fix after the first fix has been obtained.
   *
   * @return gpsOn
   */
   ...
   * negative value less than -4, it will be coerced to -14 dB, if greater than -4 but negative, -4
   * will be used. If zero or positive, it will be coerced to +6 dB.
   *
   * @return afGain
   */
   ...
   * between bursts. If set to 1, the instrument will not power down between bursts. This is useful
   * to avoid any warm-up artefacts at the start of each burst to maximise precision, but unfeasible
   * for long term deployments. This has no effect during Attended mode. A difficulty is that the
   * PLL light will not be extinguished between bursts, and there is a danger that the radar will be
   * switched off during a burst and the final data file might be corrupted.
   *
   * @return sleepMode
   */
   ...
   * Settle_Cycles greater than zero. If averaging is not switched on, there is little advantage to
   * discarding the first chirps in a burst – it can be done during post-processing.
   *
   * @return settleCycles
   */
   ...
   * that will allow the chirp properties to be changed. The tool appends the relevant lines to the
   * end of a config.ini file. Those lines are then used on power-up to overwrite the default chirp
   * configuration
   *
   * @return reg00
   */
