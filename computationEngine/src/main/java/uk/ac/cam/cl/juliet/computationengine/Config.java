...
public class Config {
   ...
   *
   * <p>When used in attended mode, and doing a Trial Sub-Burst, the maximum depth that is displayed
   * on the FFT (A-scope) display. This can be overwritten from the browser.
   *
   * <p>Relevant for Attended Mode
   *
   ...
   * Assumption is that a fault has occurred if radar is active for longer than this time. Watchdog
   * does not operate in ; attended mode. If Watchdog time is set to 0, then the default of 3600
   ...
   *
   * <p>Undertake daily housekeeping (GPS clock check, Iridium exchange and memory card check? (true
   * = yes, false = no)
   *
   * <p>Relevant for both Attended and Unattended modes
   *
   ...
   * defaults in the instrument and, like many parameters in the config file, do not need to be set
   * here. They are included for completeness.
   *
   * @return reg
   */
